...
class Training_Pipeline:
    def __init__(self, model_args, data_args, training_args):
        ...
        if isinstance(features[self.label_column_name].feature, ClassLabel):
            ...
            self.label_list = get_label_list(datasets["train"][self.label_column_name])
        ...
        config = AutoConfig.from_pretrained(
            ...
            beta=model_args.beta,  # we need to define beta with input
    ...
    def entities2dict(self, entities, queryid, ent_dict):
        ...
        for entity in entities:
            tag, index, word = entity["entity_group"], sorted(entity["index"]), entity["word"]
    ...
    def compute_metrics(self, p):
        ...
        with open("pred_triplets(bert).csv", "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerows(true_predictions)

        with open("label_triplets(bert).csv", "w", newline="") as f:
            writer = csv.writer(f)
    ...
    def handling_score(
        ...
        for idx, label_idx in filtered_labels_idx:
            word = self.tokenizer.convert_ids_to_tokens([int(input_ids[idx])])[0]  # contains "##"
    ...
    def instance_select(self, model):
        ...
        # self.trust_idx = self.matched_IDs | self.init_idx
