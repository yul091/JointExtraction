...
class BertForTokenClassification(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.use_subtoken_mask = False

        self.bert = BertModel(config, add_pooling_layer=False)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.position_attention = Position_Attention(config)  # position attention
        self.classifier = nn.Linear(2 * config.hidden_size, self.num_labels, bias=False)  # z_t = W_u * u_t
    ...
    def forward(
        ...
        sequence_output = outputs[0]  # B X T X H
        # print("bert_output: {}".format(sequence_output))

        sequence_output = self.dropout(sequence_output)

        # Training
        if query_ids is not None:
            a_t, query_hiddens = self.position_attention(sequence_output, query_ids)  # a_t: (B X T), u_t: (B X T X 2H)
            ...
            if labels is not None:
                ...
                cls_loss = self.crf(logits, labels, mask=label_mask)
                if self.beta == 0:
                    loss = cls_loss  # loss of (batch_size)
                else:
                    kl_loss = self.kl_loss(a_t.log(), target_att)
                    loss = cls_loss + self.beta * kl_loss  # loss of (batch_size)

                    print("num_labels: {}, cls_loss: {}, kl_loss: {}".format(self.num_labels, cls_loss, kl_loss))
