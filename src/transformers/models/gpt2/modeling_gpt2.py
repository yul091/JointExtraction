...
class Position_Attention2(nn.Module):
    def __init__(self, config):
        super().__init__()
        hidden_dim = config.n_embd
        self.hidden_dim = hidden_dim
    ...
    def _pa(self, hidden_states, h_p, h_t):
        ...
        hidden_states (B X T X H): hidden states of transformer output for position attention.
        """
        tail = (h_p + h_t).unsqueeze(1).expand_as(hidden_states)  # B X H -> B X T X H
        s_t = self.v(torch.tanh(hidden_states + tail))  # B X T X 1
        a_t = self.softmax(s_t)  # B X T X 1
        c_t = torch.sum(a_t.expand_as(hidden_states) * hidden_states, dim=1)  # B X H
        return a_t.squeeze(-1), c_t
...
class GPT2ForTokenClassification(GPT2PreTrainedModel):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.classifier_type = config.token_classifier_type
        self.use_dropout = config.token_classifier_dropout
        self.o_label_id = config.token_classifier_o_label_id
        self.use_subtoken_mask = config.use_subtoken_mask
        self.label2id = config.label2id
        self.id2label = config.id2label

        # Model architecture
        # GPT2Model -> (Dropout conditioned by token_classifier_dropout) -> Position Attention -> (CRF / PartialCRF conditioned by token_classifier_type)
        self.transformer = GPT2Model(config)

        if self.use_dropout:
            self.dropout = nn.Dropout(config.hidden_dropout_prob)

        self.position_attention = Position_Attention2(config)  # position attention
        self.classifier = nn.Linear(2 * config.n_embd, self.num_labels, bias=False)  # z_t = W_u * u_t

        if self.classifier_type == "crf":
            from .crf import CRF

            self.crf = CRF(self.num_labels)
        elif self.classifier_type == "partial-crf":
            from .crf import PartialCRF

            self.partial_crf = PartialCRF(self.num_labels, unlabeled_index=self.o_label_id)
    ...
    def forward(
        self,
        input_ids=None,  # embeddings of input tokens (B X T)
        past_key_values=None,
        attention_mask=None,  # attention_mask: (B X T) in [0, 1]
        token_type_ids=None,
        position_ids=None,
        head_mask=None,
        inputs_embeds=None,
        ...
        # print("gpt2_output: {}".format(hidden_states))
        if self.use_dropout:
            hidden_states = self.dropout(hidden_states)

        # Training
        if query_ids is not None:
            a_t, query_hiddens = self.position_attention(hidden_states, query_ids)  # a_t: (B X T), u_t: (B X T X 2H)
            # print("a_t: {}, u_t: {}".format(a_t, query_hiddens))
            logits = self.classifier(query_hiddens)  # (B X T X 2H) -> (B X T X num_labels)

            loss = None
            label_mask = None
            if labels is not None:
                if self.classifier_type in (
                    "crf",
                    "partial-crf",
                ):
                    if attention_mask is not None:
                        active_loss = attention_mask == 1
                        labels = torch.where(active_loss, labels, torch.tensor(-100).type_as(labels))

                    # Label may contain ignore_index (-100)
                    # Generate label mask of shape (B X T)
                    label_mask = torch.ones_like(labels, dtype=torch.uint8)
                    label_mask = torch.where(labels < 0, torch.tensor(0).type_as(label_mask), label_mask)
                    # Replace -100 in labels to avoid exception in CRF/PartialCRF
                    # Current choice: label O
                    labels = torch.where(labels < 0, torch.tensor(self.o_label_id).type_as(labels), labels)

                    # print('logits: {}, labels: {}'.format(logits, labels))

                    if self.use_subtoken_mask:
                        a_t = a_t * label_mask
                        target_att = target_att * label_mask

                    if self.classifier_type == "crf":
                        crf_loss = self.crf(logits, labels, mask=label_mask)
                        loss = crf_loss  # loss of (batch_size)

                    elif self.classifier_type == "partial-crf":
                        loss = self.partial_crf(logits, labels, mask=label_mask)
            ...
            if self.classifier_type in (
                ...
                # NOTE The effect of `mask` in `viterbi_decode`:
                # `mask` decides whether the model should compute the score for a masked token;
                # A masked token's score will be inherited from its previous token's,

                # Apply masks on subwords after tokenization
                # (1) For trainer-based prediction using run_ner.py, labels will be provided, so we only need to use label_mask computed previously;
                # (2) For pipeline-based prediction, proposed changes in token_classification.py to create masks on subword tokens.
                if label_mask is None:
                    label_mask = crf_mask
                # (B X T X num_labels) -> (B X T)
                if self.classifier_type == "crf":
                    viterbi_labels = self.crf.viterbi_decode(logits, mask=label_mask)
                elif self.classifier_type == "partial-crf":
                    viterbi_labels = self.partial_crf.viterbi_decode(logits, mask=label_mask)
                ...
                # TokenClassificationPipeline applies softmax on logits to select labels
                # For CRF/PartialCRF, CRF marginal_probability != logits,
                # so we need to incorporate viterbi decoding into this framework
                # by transforming viterbi decoded labels into "fake" linear-output logits
            if not return_dict:
                output = (logits,) + transformer_outputs[2:] + (a_t,)
                return ((loss,) + output) if loss is not None else output
        ...
        # Evaluating & Predicting
            ...
            # # save the attention for this batch to local
            # DIR = 'examples/tok_cls_result/attention/relation_base_re' # [quarter_ent, quarter_re, relation_ent, relation_re, quarter_base_ent, quarter_base_re, relation_base_ent]
            # if not os.path.exists(DIR):
            #     os.makedirs(DIR)
            # res = torch.stack(attention_stack).transpose(0, 1) # B X T X T
            # right_now = int(time.time())
            # torch.save(
            #     res,
            #     os.path.join(DIR, "attention_{}.pt".format(right_now))
            # )

            if not return_dict:
                output = (all_logits,) + transformer_outputs[2:]
                return output
