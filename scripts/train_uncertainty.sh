...
OUTPUT_DIR="examples/tok_cls_result/NYT_${MODEL_UNCERTAINTY}_${THRESHOLD}_${DATA_UNCERTAINTY}_${COMMITTEE_SIZE}"


CUDA_VISIBLE_DEVICES=2 python examples/token-classification-gpt2/run_uncertainty.py \
...
--per_device_train_batch_size 3 --per_device_eval_batch_size 20 \
--gradient_accumulation_steps 16 --overwrite_cache \
--use_negative_sampling --sample_rate 0.1 --num_train_epochs 100 \
--boot_start_epoch 5 --threshold $THRESHOLD --committee_size $COMMITTEE_SIZE \
--ensemble_weight $ENSEMBLE_WEIGHT --save_total_limit 2 \
--model_uncertainty $MODEL_UNCERTAINTY --data_uncertainty $DATA_UNCERTAINTY
